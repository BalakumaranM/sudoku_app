...
class StatsRepository {
  ...
  static Future<CategoryStats> _getCategoryStats(
    ...
    for (int level = 1; level <= levelsPerDifficulty; level++) {
      final detail = _parseLevelDetail(prefs, prefix, level);
      levelDetails.add(detail);
      
      if (detail.isCompleted) {
        completedCount++;
        totalTime += detail.timeSeconds;
        totalMistakes += detail.mistakes;
        
        if (detail.timeSeconds > 0 && (bestTime == 0 || detail.timeSeconds < bestTime)) {
          bestTime = detail.timeSeconds;
        }
      }
    }
  ...
  static LevelDetail _parseLevelDetail(SharedPreferences prefs, String prefix, int level) {
